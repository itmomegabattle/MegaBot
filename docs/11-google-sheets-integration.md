...
- при смене недели сервер сначала создаёт полную резервную копию `ОСНОВА`, затем обновляет семь дат, очищает только значения временных слотов и восстанавливает checkbox-валидацию;
...
Если даты уже относятся к текущей неделе, команда не очищает значения и не создаёт повторную копию: она только проверяет checkbox-валидацию и служебные привязки. Если неделя изменилась, резервная копия создаётся до первой мутации.
...
Fallback reconciliation запускается раз в минуту. Порядок принципиален: сначала повторяются ожидающие экспорты приложения, затем импортируются изменения таблицы. Apps Script остаётся быстрым push-каналом таблица → сервер, а минутный импорт — страховкой.
