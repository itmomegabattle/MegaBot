...
Схема версии 4 добавляет несколько блоков на одну задачу, сжатые пользовательские аватары и лист `task_log`. `task_log` предназначен для чтения командой и подготовки чек-листов будущих мероприятий; технические нормализованные листы и `snapshot` остаются источником восстановления.
...
Аварийное восстановление проверенного JSON-снимка выполняется только при остановленном боте:
...
Команда проверяет структуру файла, записывает полный снимок и сравнивает digest после обратного чтения.
...
## Состояние схемы 2026-08-09

Операционная Google Sheets database остаётся каноническим хранилищем бизнес-данных. `chat-panels.json` — только transient sidecar для Telegram message IDs, очереди и черновиков слотов; новый `dayBaseline` не мигрируется в таблицы и исчезает после завершения редактирования. Планируемые персональные задания потребуют отдельного состояния `task_assignees` и версии схемы; до миграции глобальный статус Task менять нельзя.
